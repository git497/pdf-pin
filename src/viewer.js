...
const CSS_UNITS = 96.0 / 72.0 // pdf尺寸与css尺寸换算
...
function Viewer(container, options = {}) {
  ...
  self.pinOpacity = options.pinOpacity || 1 // 图钉透明度
  self.maxScale = options.maxScale || 10
  self.minScale = options.minScale || 0.001
  ...
  function load(url) {
    ...
    function getScale(container, pdf) {
      return pdf.getPage(1)
        .then(data => {
          page = data
          viewport = page.getViewport(1)
          return container.clientWidth / (viewport.width * CSS_UNITS)
        })
    }
  }
    ...
    function addImage(url) {
      return new Promise(resolve => {
        fabric.Image.fromURL(url, img => {
          ...
          img.topRate = point.y / pinCanvas.height
          img.leftRate = point.x / pinCanvas.width
          img.opacity = self.pinOpacity
  ...
  function addPinWithRate(leftRate, topRate, pinImgURL, textOptions, extraData) {
    const viewportPt = {
      x: leftRate * pinCanvas.width,
      y: topRate * pinCanvas.height
    }
    return addPin(viewportPt, pinImgURL, textOptions, extraData)
  }
  ...
  function zoomIn(delta) {
    ...
    pinCanvas.getObjects().forEach(obj => {
      obj.set('top', pinCanvas.height * obj.topRate - obj.height)
      obj.set('left', pinCanvas.width * obj.leftRate - obj.width / 2)
      obj.setCoords()
    })
    pinCanvas.renderAll()
  ...
  function movePin(e, img) {
    img.topRate = (img.top + img.height) / pinCanvas.height
    img.leftRate = (img.left + img.width / 2) / pinCanvas.width
